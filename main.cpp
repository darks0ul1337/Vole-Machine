...
string ALU::bin_to_hex(string num) {
	bitset<8> set(num);
	stringstream res;
...
string ALU::hex_to_bin(string num) {
	string bin;
	stringstream ss;
	ss << hex << num;
	unsigned int n;
...
void CU::Add(int idxReg1, int idxReg2, int idxReg3, Memory& Register) {
	string i1 = alu.hex_to_bin(Register.getCell(idxReg2));
	...
	Register.setCell(idxReg1, alu.bin_to_hex(result));
}
...
void CU::Add(Memory& Register, int idxReg1, int idxReg2, int idxReg3) {
	...
	Float8 i1 = Float8::fromBin(alu.hex_to_bin(Register.getCell(idxReg2)));
	Float8 i2 = Float8::fromBin(alu.hex_to_bin(Register.getCell(idxReg3)));
	Float8 result = Float8::add(i1, i2);
	Register.setCell(idxReg1, alu.bin_to_hex(result.printBinary()));
}
